...
namespace DARKSTAR.Features
{
    public static class GameWindowGuardManager
    {
        ...
        private static void EnsureWinKeyHook()
        {
            ...
            // .NET 8 single-file publish bundles the app into a non-standard PE that
            // SetWindowsHookEx may not recognize via GetModuleHandle(null). Using
            // LoadLibrary("user32.dll") is the standard reliable workaround — Windows
            // only uses hMod for WH_KEYBOARD_LL to identify the registering app, and
            // user32.dll is always loaded, so this always succeeds.
        ...
        private static IntPtr KeyboardHookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                ...
                // Block both KEYDOWN and KEYUP for Win keys. Blocking only KEYDOWN
                // lets the KEYUP through, which can still trigger the Start menu
                // on some Windows configurations.
                if (message == WM_KEYDOWN || message == WM_SYSKEYDOWN ||
                    message == WM_KEYUP || message == WM_SYSKEYUP)
                {
                    var key = Marshal.PtrToStructure<KBDLLHOOKSTRUCT>(lParam);
                    // The hook is only installed when the game is active (managed by
                    // the worker thread). Don't re-check IsConfiguredGameWindowActive
                    // here — that does multiple P/Invoke calls and can cause Windows
                    // to silently remove the hook due to LowLevelHooksTimeout.
