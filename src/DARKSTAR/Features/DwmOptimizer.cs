...
namespace DARKSTAR.Features
{
    ...
    ///     → BELOW_NORMAL (-1), ALL cores, boost disabled
    ///   Using IDLE (-15) on E-cores is too aggressive and can starve the compositor,
    ///   causing presentation stalls and STATUS_UNKNOWN_HARD_ERROR.
    ///
                    ...
                    try
                    {
                        ...
                        foreach (ProcessThread thread in process.Threads)
                        {
                            try
                            {
                                ...
                                {
                                    // All other DWM threads → BELOW_NORMAL on all cores
                                    // IDLE (-15) on E-cores starves the compositor and causes
                                    // presentation stalls that increase input-to-photon latency.
                                    ApplyThreadSettings(thread.Id, -1, "ALL", threadName);
                                    demotedCount++;
                                }
                            }
                            catch { }
