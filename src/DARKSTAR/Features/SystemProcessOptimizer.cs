            ...
            lock (_lock)
            {
                ...
                foreach (var kvp in _originalStates)
                {
                    ...
                    try
                    {
                        ...
                        {
                            // Try with kernel driver for PPL-protected processes
                            if (Core.ConfigLoader.UseKernelDriver && Core.KernelDriverInterface.IsPplBypassAvailable)
                            {
                                // Restore to Normal (NT class 2)
                                byte ntNormalPriority = 2;
                                if (Core.KernelDriverInterface.SetProcessPriorityDarkstar((uint)pid, ntNormalPriority))
                                {
                                    Core.Logger.WriteVerbose($"[SYS-OPT] Restored {state.ProcessName} (PID {pid}) via kernel driver", ConsoleColor.DarkCyan);
                                }
                                else
                                {
                                    Core.Logger.WriteVerbose($"[SYS-OPT] Cannot restore {state.ProcessName} (kernel driver failed)", ConsoleColor.Yellow);
                                }
                            }
                            else
                            {
                                Core.Logger.WriteVerbose($"[SYS-OPT] Cannot restore {state.ProcessName} (protected, no kernel driver)", ConsoleColor.Yellow);
                            }
                            continue;
                        }
            ...
            foreach (var process in Process.GetProcessesByName("csrss"))
            {
                try
                {
                    ...
                    if (hProcess == IntPtr.Zero)
                    {
                        ...
                        // csrss is a protected process, use kernel driver to bypass PPL
                        if (Core.ConfigLoader.UseKernelDriver && Core.KernelDriverInterface.IsPplBypassAvailable)
                        {
                            Core.Logger.WriteVerbose($"[SYS-OPT] Using kernel driver for csrss (PID {pid})", ConsoleColor.Cyan);
                            
                            // NT internal priority class: 4 = Realtime, 3 = High
                            byte ntPriorityClass = 4; // Realtime
                            if (Core.KernelDriverInterface.SetProcessPriorityDarkstar((uint)pid, ntPriorityClass))
                            {
                                Core.Logger.WriteVerbose($"[SYS-OPT] csrss (PID {pid}) set to REALTIME via kernel driver", ConsoleColor.Green);
                                Core.Logger.WriteLog($"[SYS-OPT] csrss (PID {pid}) set to REALTIME via kernel driver");
                                
                                // Save state for restoration (use defaults since we can't query protected process)
                            ...
                            {
                                // Fallback: try High priority
                                byte ntHighPriority = 3; // High
                                if (Core.KernelDriverInterface.SetProcessPriorityDarkstar((uint)pid, ntHighPriority))
                                {
                                    Core.Logger.WriteVerbose($"[SYS-OPT] csrss (PID {pid}) set to HIGH via kernel driver", ConsoleColor.Yellow);
                                ...
                                {
                                    Core.Logger.WriteVerbose($"[SYS-OPT] Kernel driver failed to set csrss priority", ConsoleColor.Yellow);
                                }
                            }
                        }
                        continue;
            ...
            foreach (var process in Process.GetProcessesByName("dwm"))
            {
                try
                {
                    ...
                    if (hProcess == IntPtr.Zero)
                    {
                        ...
                        // dwm is a protected process, use kernel driver to bypass PPL
                        if (Core.ConfigLoader.UseKernelDriver && Core.KernelDriverInterface.IsPplBypassAvailable)
                        {
                            Core.Logger.WriteVerbose($"[SYS-OPT] Using kernel driver for dwm (PID {pid})", ConsoleColor.Cyan);
                            
                            // NT internal priority class: 1 = Idle
                            byte ntPriorityClass = 1; // Idle
                            if (Core.KernelDriverInterface.SetProcessPriorityDarkstar((uint)pid, ntPriorityClass))
                            {
                                Core.Logger.WriteVerbose($"[SYS-OPT] dwm (PID {pid}) set to IDLE via kernel driver", ConsoleColor.Green);
                                Core.Logger.WriteLog($"[SYS-OPT] dwm (PID {pid}) set to IDLE via kernel driver");
                            ...
                            {
                                // Fallback: try Below Normal (NT class 5 = Below Normal)
                                byte ntBelowNormal = 5;
                                if (Core.KernelDriverInterface.SetProcessPriorityDarkstar((uint)pid, ntBelowNormal))
                                {
                                    Core.Logger.WriteVerbose($"[SYS-OPT] dwm (PID {pid}) set to BELOW_NORMAL via kernel driver", ConsoleColor.Yellow);
                                ...
                                {
                                    Core.Logger.WriteVerbose($"[SYS-OPT] Kernel driver failed to set dwm priority", ConsoleColor.Yellow);
                                }
                            }
                        }
                        continue;
