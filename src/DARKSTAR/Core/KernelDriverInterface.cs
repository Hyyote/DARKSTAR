...
namespace DARKSTAR.Core
{
    public static class KernelDriverInterface
    {
        ...
        private static readonly uint IOCTL_DARKSTAR_SET_PROCESS_IO_PRIORITY = DARKSTAR_CTL_CODE(0x803);
        private static readonly uint IOCTL_DARKSTAR_SET_PROCESS_PAGE_PRIORITY = DARKSTAR_CTL_CODE(0x804);
        ...
        [StructLayout(LayoutKind.Sequential)]
        private struct DARKSTAR_SET_PROCESS_IO_PRIORITY_INPUT
        {
            public uint ProcessId;
        ...
        [StructLayout(LayoutKind.Sequential)]
        private struct DARKSTAR_SET_PROCESS_PAGE_PRIORITY_INPUT
        {
            public uint ProcessId;
        ...
        /// <summary>
        /// Set process I/O priority via Darkstar driver (bypasses PPL protection).
        /// Values: 0=VeryLow, 1=Low, 2=Normal, 3=High, 4=Critical
        /// Critical (4) is normally reserved for MM paging I/O but accessible from kernel mode.
        /// </summary>
        public static bool SetProcessIoPriority(uint processId, int ioPriority)
        {
            ...
            var input = new DARKSTAR_SET_PROCESS_IO_PRIORITY_INPUT
            {
                ProcessId = processId,
                IoPriority = ioPriority
            };
            
            IntPtr inputPtr = Marshal.AllocHGlobal(Marshal.SizeOf<DARKSTAR_SET_PROCESS_IO_PRIORITY_INPUT>());
            try
            {
                Marshal.StructureToPtr(input, inputPtr, false);
                ...
                bool success = DeviceIoControl(
                    _driverHandle,
                    IOCTL_DARKSTAR_SET_PROCESS_IO_PRIORITY,
                    inputPtr,
                    (uint)Marshal.SizeOf<DARKSTAR_SET_PROCESS_IO_PRIORITY_INPUT>(),
                    IntPtr.Zero,
                    0,
                    out uint bytesReturned,
                    IntPtr.Zero);
                ...
                {
                    int error = Marshal.GetLastWin32Error();
                    Logger.WriteVerbose($"[DARKSTAR] SetProcessIoPriority failed: Win32 error {error}", ConsoleColor.DarkGray);
                }
        ...
        /// <summary>
        /// Set process page priority via Darkstar driver (bypasses PPL protection).
        /// Values: 0=Lowest, 1=VeryLow, 2=Low, 3=Medium, 4=BelowNormal, 5=Normal(max)
        /// </summary>
        public static bool SetProcessPagePriority(uint processId, int pagePriority)
        {
            ...
            var input = new DARKSTAR_SET_PROCESS_PAGE_PRIORITY_INPUT
            {
                ProcessId = processId,
                PagePriority = pagePriority
            };
            
            IntPtr inputPtr = Marshal.AllocHGlobal(Marshal.SizeOf<DARKSTAR_SET_PROCESS_PAGE_PRIORITY_INPUT>());
            try
            {
                Marshal.StructureToPtr(input, inputPtr, false);
                ...
                bool success = DeviceIoControl(
                    _driverHandle,
                    IOCTL_DARKSTAR_SET_PROCESS_PAGE_PRIORITY,
                    inputPtr,
                    (uint)Marshal.SizeOf<DARKSTAR_SET_PROCESS_PAGE_PRIORITY_INPUT>(),
                    IntPtr.Zero,
                    0,
                    out uint bytesReturned,
                    IntPtr.Zero);
                ...
                {
                    int error = Marshal.GetLastWin32Error();
                    Logger.WriteVerbose($"[DARKSTAR] SetProcessPagePriority failed: Win32 error {error}", ConsoleColor.DarkGray);
                }
