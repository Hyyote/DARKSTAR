...
namespace DARKSTAR.Core
{
    public static class MonitoringEngine
    {
        ...
        private const int FAST_CHECK_INTERVAL = 1000;     // Was 500ms — too much handle churn
        private const int SLOW_CHECK_INTERVAL = 5000;
        private const int SYSTEM_PROCESS_CHECK_INTERVAL = 10000;
        private const int THREAD_REANALYSIS_INTERVAL = 30000;  // Was 15s — too aggressive, causes stutter
        ...
        private static DateTime _lastThreadReanalysis = DateTime.MinValue;
        ...
        private static void MonitoringLoop()
        {
            ...
            if (ConfigLoader.UseKernelDriver)
            {
                ...
                else
                {
                    ...
                    Logger.WriteMinimal("  Load DarkstarDriver.sys for kernel-mode thread priority and affinity control", ConsoleColor.Yellow);
                }
            }
            ...
            while (true)
            {
                try
                {
                    ...
                    if ((now - _lastSlowCheck).TotalMilliseconds >= SLOW_CHECK_INTERVAL)
                    {
                        ...
                        Features.GpuPriorityManager.ReapplyGpuPriorities();  // Moved from fast check — every 5s is sufficient
                        _lastSlowCheck = now;
                    }
                    ...
                    // Periodic thread re-analysis: games create/destroy threads dynamically.
                    // New render threads, worker pool threads, etc. need to be detected.
                    if (_gameOptimized && _gameProcessId > 0 &&
                        (now - _lastThreadReanalysis).TotalMilliseconds >= THREAD_REANALYSIS_INTERVAL)
                    {
                        ReanalyzeGameThreads();
                        IntelligentThreadDetector.PruneStaleHistory();
                        _lastThreadReanalysis = now;
                    }
        ...
        private static void CheckForConfiguredProcesses()
        {
            ...
            if (processedGameThisLoop && !_gameOptimized)
            {
                ...
                // Apply system process optimizations (csrss, dwm) if enabled
                if (ConfigLoader.SystemProcessOptimizerEnabled)
                {
                    try
                    {
                ...
                {
                    Logger.WriteVerbose("System process optimizer disabled in config", ConsoleColor.DarkCyan);
                }

                if (ConfigLoader.DpcCore0LockEnabled && ConfigLoader.UseKernelDriver && KernelDriverInterface.IsAvailable)
                {
                    bool dpcLockApplied = KernelDriverInterface.SetDpcCore0Lock(true);
                    Logger.WriteLog(dpcLockApplied
                        ? "[DPC] Core0 lock enabled for active game session"
                        : "[DPC] Failed to enable Core0 DPC lock");
                }

                // Close configured background processes (explorer.exe, browsers, etc.)
                // and start event-driven respawn prevention
                try
                {
                    Features.BackgroundProcessCloser.CloseForGameSession();
                }
                catch (Exception ex)
                {
                    Logger.WriteLog($"[BG-CLOSER] ERROR closing background processes: {ex.Message}");
                }

                // Start memory drain loop if enabled
                try
                {
                    Features.MemoryDrainer.StartForGameSession();
                }
                catch (Exception ex)
                {
                    Logger.WriteLog($"[MEM-DRAIN] ERROR starting memory drain: {ex.Message}");
                }
        ...
        private static void CheckPendingGameInitializations()
        {
            ...
            lock (_pendingGameInitLock)
            {
                ...
                foreach (var kvp in _pendingGameInit)
                {
                    ...
                    double elapsedSeconds = (now - initTime).TotalSeconds;

                    // Priming pass: collect baseline thread metrics at ~5 seconds so that
                    // when we optimize at 10 seconds, the behavioral scoring has real deltas
                    // instead of all-zeros. This makes the first real classification meaningful.
                    if (elapsedSeconds >= 5 && elapsedSeconds < 10)
                    {
                        try
                        {
                            if (!process.HasExited)
                            {
                                IntelligentThreadDetector.DetectGameThreads(processId, processName);
                            }
                        }
                        catch { }
                        continue;
                ...
                try
                {
                    ...
                    if (isGame)
                    {
                        ...
                        targetProcess.Exited += (sender, e) =>
                        {
                            ...
                            // Clear thread detection history for the exited game
                            IntelligentThreadDetector.ClearHistory();
                            ...
                            if (ConfigLoader.DpcCore0LockEnabled && ConfigLoader.UseKernelDriver && KernelDriverInterface.IsAvailable)
                            {
                                bool dpcLockRestored = KernelDriverInterface.SetDpcCore0Lock(false);
                                Logger.WriteLog(dpcLockRestored
                                    ? "[DPC] Core0 lock disabled after game exit"
                                    : "[DPC] Failed to disable Core0 DPC lock");
                            }
                            
                            // Restore background processes (restart explorer.exe, stop respawn guard)
                            try
                            {
                                Features.BackgroundProcessCloser.RestoreAfterGameSession();
                            }
                            catch (Exception ex)
                            {
                                Logger.WriteLog($"[BG-CLOSER] ERROR restoring background processes: {ex.Message}");
                            }

                            // Stop memory drain loop
                            try
                            {
                                Features.MemoryDrainer.StopAfterGameSession();
                            }
                            catch (Exception ex)
                            {
                                Logger.WriteLog($"[MEM-DRAIN] ERROR stopping memory drain: {ex.Message}");
                            }
        ...
        private static void ApplyIntelligentThreadOptimizations(Process process, string processName, ProcessConfig processConfig, HashSet<string> configuredThreads)
        {
            ...
            {
                Logger.WriteVerbose($"[ThreadDetector] No threads classified for {processName} on this pass", ConsoleColor.DarkCyan);
                Logger.WriteLog($"[ThreadDetector] No threads classified for {processName} on this pass");
                return;
            }
            ...
            foreach (var thread in detectedThreads)
            {
                ...
                bool isAudio = thread.Type == IntelligentThreadDetector.ThreadType.Audio;
                ...
                bool isIO = thread.Type == IntelligentThreadDetector.ThreadType.IO;
                bool isBackground = thread.Type == IntelligentThreadDetector.ThreadType.Background;

                if (!isMain && !isRender && !isAudio && !isWorker && !isNetwork && !isIO && !isBackground)
                    continue;
                ...
                {
                    // Audio threads get highest priority on P-cores for glitch-free output
                    targetPriority = THREAD_PRIORITY_HIGH_VALUE;
                    targetAffinity = pCoreAffinity;
                    audioCount++;
                }
                ...
                {
                    targetPriority = -2; // THREAD_PRIORITY_LOWEST
                    targetAffinity = eCoreAffinity;
                    backgroundCount++;
                }
                else
                ...
                if (ApplyThreadSettings(thread.ThreadId, targetPriority, targetAffinity, processConfig.DisableBoost))
                {
                    ...
                    // Use kernel driver boost for render threads (short priority spike at frame start)
                    if (isRender && ConfigLoader.UseKernelDriver && KernelDriverInterface.IsAvailable)
                    {
                        KernelDriverInterface.BoostThread(thread.ThreadId, 2, 2);
                    }

                    // Set ideal processor hints via kernel driver for critical threads
                    if ((isMain || isRender || isAudio) && ConfigLoader.UseKernelDriver && KernelDriverInterface.IsAvailable)
                    {
                        if (CpuTopologyDetector.PCores.Count > 0)
                        {
                            // Spread main/render/audio across different P-cores
                            int coreIndex = (mainCount + renderCount + audioCount - 1) % CpuTopologyDetector.PCores.Count;
                            byte idealCore = (byte)CpuTopologyDetector.PCores.OrderBy(x => x).ElementAt(coreIndex);
                            KernelDriverInterface.SetThreadIdealProcessor(thread.ThreadId, idealCore);
                        }
                    }
                }
            }

            string appliedSummary = $"Thread detector applied: Main {mainCount}, Render {renderCount}, Audio {audioCount}, Worker {workerCount}, Network {networkCount}, IO {ioCount}, Background {backgroundCount}, Updated {updated}, Skipped {skippedConfigured}";
        ...
        /// <summary>
        /// Re-analyze game threads periodically to catch dynamically created threads.
        /// Games constantly spin up/down worker threads, render threads change, etc.
        /// The first pass (at game detection time) often misses threads because
        /// behavioral data needs multiple samples to be meaningful.
        /// </summary>
        private static void ReanalyzeGameThreads()
        {
            if (_gameProcessId <= 0)
                return;
            ...
            try
            {
                var process = Process.GetProcessById(_gameProcessId);
                if (process.HasExited)
                    return;

                string processName = process.ProcessName;

                // Run thread detection — this builds on historical timing data
                var threads = IntelligentThreadDetector.DetectGameThreads(_gameProcessId, processName);

                // Only touch threads we're confident about (>= 50)
                int applied = 0;
                foreach (var t in threads.Where(t => t.ConfidenceScore >= 50 && t.Type != IntelligentThreadDetector.ThreadType.Unknown))
                {
                    try
                    {
                        IntPtr hThread = OpenThread(THREAD_SET_INFORMATION | THREAD_QUERY_INFORMATION, false, (uint)t.ThreadId);
                        if (hThread == IntPtr.Zero)
                            continue;

                        try
                        {
                            // Apply affinity recommendation
                            string affinityStr = t.CoreRecommendation == "P-Core" ? "p-core" :
                                                 t.CoreRecommendation == "E-Core" ? "e-core" : "ALL";
                            IntPtr affinity = AffinityParser.Parse(affinityStr);
                            if (affinity != IntPtr.Zero)
                            {
                ...
                if (applied > 0)
                {
                    Logger.WriteVerbose($"[ThreadReanalysis] Applied affinity to {applied}/{threads.Count} threads for {processName}", ConsoleColor.DarkCyan);
                }
            }
            catch (Exception ex)
            {
                Logger.WriteLog($"[ThreadReanalysis] Error: {ex.Message}");
            }
        }
