...
namespace DARKSTAR.Core
{
    public static class MonitoringEngine
    {
        ...
        private const int THREAD_REANALYSIS_INTERVAL = 30000;  // Re-analyze game threads every 30s
        ...
        private static void CheckForConfiguredProcesses()
        {
            ...
            if (processedGameThisLoop && !_gameOptimized)
            {
                ...
                // Close configured background processes and start respawn guard
                try
                {
                    Features.BackgroundProcessCloser.Configure(true, ConfigLoader.SystemConfigs.Keys);
                    Features.BackgroundProcessCloser.CloseForGameSession();
                }
                catch (Exception ex)
                {
                    Logger.WriteLog($"[BG-CLOSER] ERROR: {ex.Message}");
                }
                ...
                try
                {
                    ...
                    if (isGame)
                    {
                        ...
                        targetProcess.Exited += (sender, e) =>
                        {
                            ...
                            // Stop process respawn guard and restore explorer
                            try
                            {
                            ...
                            {
                                Logger.WriteLog($"[BG-CLOSER] ERROR restoring: {ex.Message}");
                            }
