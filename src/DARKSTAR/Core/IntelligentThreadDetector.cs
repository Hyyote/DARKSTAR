...
namespace DARKSTAR.Core
{
    ...
    public static class IntelligentThreadDetector
    {
        ...
        /// Prefers the foreground window if it belongs to our process, otherwise
        /// uses the first visible window found during enumeration.
        /// </summary>
        private static int FindWindowOwnerThread(int processId)
        {
            // First check: is the foreground window ours? If so, use it directly —
            // this is the most reliable signal and avoids the full enumeration.
            ...
            if (fgWindow != IntPtr.Zero)
            {
                ...
                if (fgPid == (uint)processId)
                {
                    ...
                    return (int)fgThreadId;
                }
            }

            // Fallback: enumerate windows and stop at the first visible one
            // belonging to our process. Returning false from the callback stops
            // enumeration, so we get the first match, not an arbitrary last one.
            int ownerThreadId = 0;
            ...
            EnumWindows((hWnd, lParam) =>
            {
                ...
                return false; // Stop enumeration — first visible match is enough
            }, IntPtr.Zero);
        ...
        /// the minimum threshold. Falls back to CPU-based heuristics.
        /// Skips behavioral refinement on the first pass (no deltas yet) to avoid
        /// overriding correct name-based classifications with garbage zero-scores.
        /// </summary>
        private static void RefineClassificationFromScores(GameThread thread)
        {
            var m = thread.Metrics!;

            // On the first sample pass, all deltas are zero — behavioral scores are meaningless.
            // Skip behavioral refinement entirely and rely on name/window/priority classification.
            if (m.TotalTimeDelta == 0 && m.CyclesDelta == 0 && m.CpuUsagePercent == 0.0)
                return;
