...
- **Core Scrambling** - Rotates affinity across cores 2,4,6,8 at configurable intervals (default 2000ms). Disabled by default via EnableScramble=false
...
|---------|-------------|---------|
| `UpdateTimeout` | Main loop interval in milliseconds.  Higher values reduce churn; minimum is 50ms. | `1000` |
| `ExplorerKillTimeout` | Explorer watchdog interval while a game is active. | `60000` |
| `EnableKillExplorer` | Terminate `explorer.exe` during gaming; restore afterwards. | `true` |
...
[cs2]
module=cs2.exe*, [1], pcore, disableboost, applyonce
